...
df = pd.read_csv("test-dataset/country_resume.csv", sep=";")
df_internet = pd.read_csv("test-dataset/internet.csv")
...
def graphMap():
    ...
    fig.update_traces(
        hovertemplate="<b>%{customdata[0]}</b><br>Percentage: %{customdata[1]}%<br>population: %{customdata[2]}"
    )
    return fig
...
def graphLine(df):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            name="Original data",
            marker=dict(color="#286a79", size=5),
        )
    )
    ...
    if index >= 0:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=y_fit,
                mode="lines",
                name=f"Prediction line (Grade {best_degree})",
                line=dict(color="#0090b0", width=1),
            )
        )
...
title = html.Div(
    [
        html.H4(
            "Internet Usage by Country and Projection to Achieve High Digital Inclusion."
        ),
    ],
...
footer = html.Div(
    [
        html.Div(
            ...
            className="py-0 my-0 text-success",
        ...
        html.Div(
            ...
            className="py-0 my-0 text-primary",
            style={"fontSize": "smaller"},
        ),
        html.Div(
            [
            ...
            className="py-0 my-0 text-danger",
            style={"fontSize": "smaller"},
        ),
    ],
    className="p-3",
    style={"fontSiza": "smaller"},
)

main = html.Div(
    [
        html.Div(
            [dcc.Graph(id="id-map", figure=graphMap(), config=config)],
            className="col-sm-6",
        ),
        html.Div(
...
@app.callback(Output("id-content", "children"), Input("id-map", "clickData"))
                ...
                [
                    ...
                    html.Div(
                        [
                            html.Small(
                                [
                                    html.I(className="bi bi-router me-2"),
                                    percent,
                                    html.I(className="bi bi-percent me-1"),
                                ],
                                className="py-0 my-0 text-secondary",
                            ...
                            html.Small(
                                ...
                                className="py-0 my-0 text-secondary mx-4",
                                style={"fontSize": "medium"},
                            ),
                            html.Small(
                                [
